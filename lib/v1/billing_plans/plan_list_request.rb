# This class was generated on Wed, 31 Jan 2018 15:11:20 PST by version 0.1.0-dev+6a771d-dirty of Braintree SDK Generator
# plan_list_request.rb
  ...
  module V1
    module BillingPlans
      ...
      class PlanListRequest
        ...
        def page(page)
          param = page.to_s
          @path += "page=#{URI.escape(param)}&"
        end

        def page_size(page_size)
          param = page_size.to_s
          @path += "page_size=#{URI.escape(param)}&"
        end

        def status(status)
          param = status.to_s
          @path += "status=#{URI.escape(param)}&"
        end

        def total_required(total_required)
          param = total_required.to_s
          @path += "total_required=#{URI.escape(param)}&"
        end
      end
    end
  end
