  ...
  module V1
    module CustomerDisputes
      ...
      class DisputeListRequest
        ...
        def dispute_state(dispute_state)
          param = dispute_state.to_s
          @path += "dispute_state=#{CGI::escape(param)}&"
        end

        def disputed_transaction_id(disputed_transaction_id)
          param = disputed_transaction_id.to_s
          @path += "disputed_transaction_id=#{CGI::escape(param)}&"
        end

        def next_page_token(next_page_token)
          param = next_page_token.to_s
          @path += "next_page_token=#{CGI::escape(param)}&"
        end
